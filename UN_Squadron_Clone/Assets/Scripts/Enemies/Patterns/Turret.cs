...
public class Turret : Enemy
{
    ...
    protected EnemySprites _sprites;
    ...
    protected void ChangeTurretSprite(Vector3 dir)
    {
        ...
        if (angle >= 0 && angle < 20)
        {
            _spriteRenderer.sprite = _sprites.enemySprites[0];
            _angleToShoot = 180;
        }
        else if (angle >= 20 && angle < 40)
        {
            _spriteRenderer.sprite = _sprites.enemySprites[1];
            _angleToShoot = 150;
        }
        else if (angle >= 40 && angle < 60)
        {
            _spriteRenderer.sprite = _sprites.enemySprites[2];
            _angleToShoot = 135;
        }
        else if (angle >= 60 && angle < 80)
        {
            _spriteRenderer.sprite = _sprites.enemySprites[3];
            _angleToShoot = 120;
        }
        else if (angle >= 80 && angle < 100)
        {
            _spriteRenderer.sprite = _sprites.enemySprites[4];
            _angleToShoot = 90;
